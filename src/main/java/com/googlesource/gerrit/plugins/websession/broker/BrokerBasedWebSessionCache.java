...
public class BrokerBasedWebSessionCache implements Cache<String, WebSessionManager.Val> {
  ...
  protected void processMessage(EventMessage message) {
    if (!WebSessionEvent.TYPE.equals(message.getEvent().getType())) {
      logger.atWarning().log(
          "Skiping web session message of unknown type:{}", message.getEvent().getType());
      return;
    }

    WebSessionEvent event = (WebSessionEvent) message.getEvent();
    if (WebSessionEvent.Operation.ADD.equals(event.operation)) {
    ...
    } else if (WebSessionEvent.Operation.REMOVE.equals(event.operation)) {
  ...
  private void sendEvent(String key, Val value) {
    ...
    try (ByteArrayOutputStream out = new ByteArrayOutputStream();
      ...
      EventMessage message =
          brokerApi
              ...
              .newMessage(
                  UUID.randomUUID(),
                  new WebSessionEvent(key, serializedObject, WebSessionEvent.Operation.ADD));
